...
<!-- Cart Page Start -->
{{--
<div class="container-flui py-5">
    <div class="container py-5 rtl">
        <div class="row" id="cart_ite">
           @forelse ($getCartItems as $cart)
                @foreach ($cart->cartItems as  $cartItem)
                <div class="col-md-12 col-12 col-lg-6 m-2 col-xl-6 pt-4   rounded bg-soDark recomnded-item position-relative" id="item-{{346*$cartItem->id}}">
        ...
        <div class="row g-4 text-center align-items-center justify-content-center pt-4">
            <button   type="submit" @if($hasCart<1) disabled @endif class="btn border-secondary py-3 px-4 text-uppercase w-100 next-step text-secondary">متابعة عملية الشراء</button>
        </div>
    </div>
</div>
--}}
...
<div class="container-fluid py-5">
    <div class="container py-5">
        <div class="row g-4">
            @forelse ($getCartItems as $cart)
                ...
                <div class="col-md-6 col-xl-6">
                    <div class="pt-4 py3 px-2 cart-totally  rounded recomnded-item position-relative">
                        <div class="position-absolute rounded-pillborder border-secondary  px-2 py-1" onclick="removeCartItem({{$cartItem->id}})" style="top: 4px;left: 4px;" >
                            <i class="fa fa-times text-danger"></i>
                        </div>
                        <div class="row align-items-center    ">
                            <div class="col-lg-5 col-12  pb-3 border-5 mt-3"  >
                            @foreach ($cartItem->product->images as $product_images)
                                @if ($loop->first)
                                    @foreach ($product_images->path as $img)
                                        @if ($loop->first)
                                            <div class="px-5 px-lg-2 px-sm-6 px-md-5 "  >
                                                <img src="{{asset("storage/$img")}}" class="img-fluid rounded-circle  mt-1"
                                                     style="border: 5px solid #ffbe33;" alt="">
                                            </div>
                                        @endif
                                    @endforeach
                            ...
                            @endforeach
                                ...
                                <div class="col-12 col-lg-7 brands px-3 px-md-4">
                                    <h6 class="text-primary product-name text-end pe-2">{{$cartItem->product->product_name}}</h6>
                                    <div class="py-2">
                                        <span class="text-primary"> سعر الوحدة :</span>
                                        <span class="text-primary fs-7 fw-bold">{{$cartItem->price}} ر.س</span>
                                        <del class="text-danger fs-8 p-1">٦٩ ر.س</del>
                                    </div>
                                    <div class="d-flex justify-content-between flex-lg-wrap px-md py-2">
                                        <div class="d-flex justify-content-end my-2">
                                    ...
                                    <div class="py-1 d-flex">
                                        ...
                                        <div class="input-group quantity  ms-3 me-3" style="width: 100px;">
                                            ...
                                            <input type="text" id="item{{$cartItem->id}}Quantity" class="form-control form-control-sm text-center bg- text-primary border-0"
                                                   value="{{$cartItem->quantity}}">
                                            <div class="input-group-btn">
                                                <button class="btn btn-sm btn-plus rounded-circle bg-secondary px-2 py-" onclick="updateCartItem({{$cartItem->id}},1)">
                                                    <i class="fa text-white fa-plus"></i>
        ...
        <div class="row g-4 justify-content-end my-5">
            <div class="col-8"></div>
            <div class="col-sm-8 col-md-7 col-lg-6 col-xl-4">
                <div class="cart-totally rounded">
                    <div class="p-4">
                        <h1 class="display-6 mb-4">السلة <span class="fw-normal">اجمالي</span></h1>
                        <div class="d-flex justify-content-between mb-4">
                            <h5 class="mb-0 ms-4">مبلغ الطلب:</h5>
                    ...
                    <div class="p-3 text-center ">
                        <a  href="@if($hasCart>0)/address
                             @else #
                           @endif ">
                            <button   type="button"  class="btn border-secondary bg-primary py-2-5 px-3 text-uppercase w-100 next-step text-secondary">متابعة عملية الشراء</button>
                        </a>
                    </div>
                </div>
            </div>

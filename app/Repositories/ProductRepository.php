...
class ProductRepository implements \App\interfaces\ProductInterface
{
    ...
    public function getCollectionsProducts($request,$scope=null): \Illuminate\Database\Eloquent\Builder
    {
        if ($scope=='search'){
            $scope=null;
        }
        ...
        return \App\Models\Product::with([
            ...
            $query->whereFullText(['product_name','description','keyword','old_price','new_price'],$searched_value)->orWhereRaw("'".$searched_value."'=''");})
    ...
    public function getAdvertisementProducts($request,$advertisementId): \Illuminate\Database\Eloquent\Builder
    {
        ...
        return \App\Models\Product::with([
            ...
            ->where(function($query) use($q_colors){
                $query->whereRelation('images','color_id',explode(',',$q_colors))->orWhereRaw("'".$q_colors."'=''");})
            ->where('new_price','>', intval($q_maxPrice))
            ->whereRelation('advertisements','advertisement_id','=',$advertisementId)->take(100)->select(['id','product_name','old_price','new_price','keyword','brand_id'])->orderBy($order->o_column,$order->o_order);
